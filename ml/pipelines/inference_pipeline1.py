...
def detect_suppression(traits: dict) -> dict:
    flags = []
    level = round(traits.get("suppression_signal", 0) * 10)
    pressure = round(traits.get("pressure_conformity", 0) * 10)
    childhood = traits.get("childhood_divergence", 0)
    fear = traits.get("fear_avoidance", 0)
    self_aw = traits.get("self_awareness", 0)

    if traits.get("suppression_signal", 0) > 0.55 and traits.get("pressure_conformity", 0) > 0.5:
        flags.append({
            "type": "external_pressure",
            "title": "External Pressure Detected",
            "insight": "Your answers suggest your current path may have been shaped more by others' expectations than your own. This is worth reflecting on."
        })

    if childhood > 0.5 and traits.get("intrinsic_motivation", 0) < 0.4:
        flags.append({
            "type": "childhood_gap",
            "title": "Childhood Dream Gap",
            "insight": "There's a notable distance between what you may have wanted as a child and where you're headed. That divergence often signals suppressed potential."
        })

    if fear > 0.55 and traits.get("risk_appetite", 0) < 0.3:
        flags.append({
            "type": "fear_driven_path",
            "title": "Fear-Driven Choices",
            "insight": "Your responses suggest some of your career choices may be driven more by avoiding failure than by pursuing what excites you."
        })

    if traits.get("creativity", 0) > 0.55 and traits.get("passion_signal", 0) > 0.4 and traits.get("depth_focus", 0) > 0.4:
        if traits.get("suppression_signal", 0) > 0.4:
            flags.append({
                "type": "creative_suppression",
                "title": "Creative Potential Suppressed",
                "insight": "You show strong creative and innovative thinking, but may not be channeling it fully. This is one of the most common sources of career dissatisfaction."
            })
    ...
    if self_aw < 0.25:
        flags.append({
            ...
            "insight": "Your responses suggest you may not yet have a clear picture of your own strengths and motivations. This is the starting point — not a weakness."
        })
