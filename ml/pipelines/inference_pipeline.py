"""
WDIG ML Inference Pipeline v5.0 — with Self-Learning
======================================================
Changes from v4.0:
  - load_adjustments() called on every predict() to apply learned career factors
  - save_feedback() triggers run_retraining() when count crosses 100n boundary
  - All original logic is preserved exactly — self-learning is additive only
"""
...
def detect_suppression(traits: dict) -> dict:
    ...
    if creativity > 0.5 and suppression > 0.35:
        flags.append({
            ...
            "insight": "You show strong creative thinking, but your answers suggest you may not be channeling it fully — possibly due to external pressure or self-doubt."
        })
    ...
    if suppression > 0.7 and not any(f["type"] == "external_pressure" for f in flags):
        flags.append({
            ...
            "insight": "Your overall response pattern shows a very high suppression signal — meaning significant parts of your interests or potential may be being held back."
        })
    ...
    if self_aw < 0.3:
        flags.append({
            ...
            "insight": "Your responses suggest you may not yet have a clear picture of your own strengths and motivations. Awareness is the first step."
        })
...
CAREER_MATRIX = {
    ...
    "Chartered Accountant / Financial Analyst": {
        ...
        "society_role": "Ensures financial systems work honestly and efficiently",
...
def rank_all_careers(traits: dict, adjustments: dict = None) -> list:
    """
    Score all careers. If adjustments dict is provided (from self-learning),
    multiply base score by the learned factor before sorting.
    """
    ...
    for career, config in CAREER_MATRIX.items():
        ...
        # Penalty for unmet required traits
        penalty = 1.0
        ...
        # This is the only new line vs the original rank_all_careers.
        # adjustments[career] is a factor learned from user feedback (default 1.0).
        learned_factor = adjustments.get(career, 1.0)

        final_score = round(score * penalty * learned_factor, 1)
...
def get_dimension_scores(traits: dict) -> dict:
    dimensions = {
        ...
        "personality":  ["discipline", "resilience", "independence", "adaptability",
                         "growth_mindset", "risk_appetite", "depth_focus", "confidence",
                         "stress_tolerance", "accountability", "initiative", "problem_solving"],
        "motivational": ["intrinsic_motivation", "purpose_drive", "passion_signal",
                         "fear_avoidance", "learning_orientation"],
        "social":       ["communication", "leadership", "teamwork", "empathy",
                         "emotional_intelligence", "helping_orientation"],
        "suppression":  ["suppression_signal", "pressure_conformity",
...
def init_db():
    """Ensure feedback table exists (self_learning.py manages the other tables)."""
    conn = _db_connect()
    c = conn.cursor()
    c.execute("""
...
def save_feedback(firebase_uid, top_career, thinking_style_id,
                  rating, comment, traits) -> None:
    """
    Save feedback to DB. After saving, check if we've crossed a 100n boundary
    and trigger retraining if so.
    """
    init_db()
    conn = _db_connect()
    c = conn.cursor()
    c.execute("""
        INSERT INTO feedback
            (firebase_uid, top_career, thinking_style_id, rating, comment, traits, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (firebase_uid, top_career, thinking_style_id, rating, comment,
          json.dumps(traits), datetime.utcnow().isoformat()))
    conn.commit()
    conn.close()
    ...
    try:
        from self_learning import should_retrain, run_retraining
        if should_retrain():
            logger.info("Feedback count crossed 100n boundary — triggering retrain...")
            run_retraining()
    except Exception as exc:
        # Never crash the feedback endpoint because of a retrain error
        logger.error(f"Retrain trigger failed (non-fatal): {exc}")


def _load_adjustments() -> dict:
    """
    Load self-learned career score adjustments from the DB.
    Returns empty dict if self_learning module or table unavailable.
    """
    try:
        from self_learning import load_adjustments
...
def predict(traits: dict, firebase_uid: str = None) -> dict:
    ...
    # Load self-learned adjustments and apply to career ranking
    adjustments       = _load_adjustments()
    all_careers       = rank_all_careers(traits, adjustments=adjustments)
    ...
    return {
        ...
        "version":           "5.1-self-learning",
        # Include meta info so frontend can show a "results refined" badge
        "self_learning_meta": {
            "adjustments_active": len(adjustments),
            "careers_with_boost": [k for k, v in adjustments.items() if v > 1.0],
            "careers_with_penalty": [k for k, v in adjustments.items() if v < 1.0],
        }
    }