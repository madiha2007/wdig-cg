...
// Checks if the user has filled out their profile.
// If not → shows a full-screen blocking modal (no skip option).
...
export default function ProfileGate({ children }: Props) {
  ...
  useEffect(() => {
    const unsub = auth.onAuthStateChanged(async (user) => {
      ...
      try {
        ...
        // If the API is unreachable, let them through rather than blocking forever
        setStatus("ok");
      }
    });
    return () => unsub();
  ...
  if (status === "missing")  return <ProfileMissingModal />;
  return <>{children}</>;
}
...
/* ── Blocking modal — no skip ────────────────────────────────────────────── */
function ProfileMissingModal() {
  const router = useRouter();
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 520, width: "100%", animation: "fadeUp .5s cubic-bezier(.16,1,.3,1) both" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          ...
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "0.9rem", lineHeight: 1.75, maxWidth: 400, margin: "0 auto" }}>
            Your aptitude test results are only as good as the context behind them.
            A 2-minute profile makes your report{" "}
            <em style={{ color: "rgba(255,255,255,0.75)" }}>dramatically more personal</em> — your career matches, roadmap, and advice will all be tailored to your real situation.
          </p>
        </div>
        ...
        <div style={{
          ...
          <p style={{ fontSize: "0.8rem", color: "rgba(255,255,255,0.65)", lineHeight: 1.7, margin: 0 }}>
            ...
            <strong style={{ color: T.tealMid }}>personalised snapshot</strong> — 2–3 sentences written specifically about your situation, dream, and what's holding you back. Without a profile, this section won't exist.
          </p>
        </div>
        ...
        <button
          ...
          style={{
            ...
            border: "none", color: T.white, padding: "1rem",
        ...
        <p style={{ textAlign: "center", color: "rgba(255,255,255,0.2)", fontSize: "0.66rem", marginTop: "1rem", lineHeight: 1.5 }}>
          Your profile is saved — you only need to do this once.
        </p>
      </div>
    </div>
  );
