...
// Derive societal offering blurb from traits + top careers
function buildSocietalBlurb(normalizedTraits = {}, topCareers = [], thinkingStyle = "") {
  ...
  let opening = "";
  if (impact > 70 && empathy > 65)
    opening = "You carry an unusual blend of empathy and awareness — the kind that notices what others overlook.";
  else if (innovation > 72)
    opening = "Your mind naturally gravitates toward building things that don't yet exist.";
  else if (leadership > 68)
    opening = "People tend to look to you, often before you've even offered to lead.";
  else if (helping > 70)
    opening = "At your core, you're someone who finds meaning in making things easier for others.";
  else
    opening = `Your profile as a ${thinkingStyle || "thinker"} positions you for meaningful impact.`;
  ...
  let close = "";
  if (impact > 65)
    close = "The world will feel your contribution most when you stop waiting for permission to act on what you already sense is important.";
  else if (innovation > 65)
    close = "Your edge is originality — use it before the world standardises what you're imagining today.";
  else
    close = "Your biggest impact is still ahead, and it starts with the choices you're making right now.";

  return `${opening} ${middle} ${close}`;
}

// Derive skill cards from traits + careers
function buildSkillCards(normalizedTraits = {}, topCareers = [], dominantTraits = []) {
  ...
  const existing = dominantTraits
    .filter(t => t.score >= 65 && SKILL_MAP[t.trait])
    .slice(0, 5)
    .map(t => ({ label: SKILL_MAP[t.trait] || humanLabel(t.trait), score: t.score }));

  // Skills to acquire: look at what the top careers typically need vs low-scoring traits
  const allCareerDomains = (topCareers ?? []).slice(0, 3).map(c => c.domain ?? "");
  const isTech = allCareerDomains.some(d => /tech|engineer|data|software/i.test(d));
  const isBusiness = allCareerDomains.some(d => /business|finance|consult|manage/i.test(d));
  const isCreative = allCareerDomains.some(d => /design|art|media|creat/i.test(d));
  const isScience = allCareerDomains.some(d => /science|research|med|bio|chem/i.test(d));
  const isLaw = allCareerDomains.some(d => /law|legal|policy/i.test(d));

  const acquire = [];
  if (isTech) acquire.push("Computational Thinking", "Data Literacy", "Systems Design");
  if (isBusiness) acquire.push("Strategic Planning", "Financial Literacy", "Stakeholder Management");
  if (isCreative) acquire.push("Visual Communication", "Prototyping", "User Research");
  if (isScience) acquire.push("Research Methodology", "Statistical Analysis", "Scientific Writing");
  if (isLaw) acquire.push("Legal Reasoning", "Argumentation", "Policy Analysis");

  // Fill with generic growth areas from low traits
  const lowTraits = Object.entries(normalizedTraits)
    .filter(([k, v]) => v < 0.45 && SKILL_MAP[k])
    .sort((a, b) => a[1] - b[1])
    .slice(0, 3)
    .map(([k]) => SKILL_MAP[k]);

  const acquireSet = [...new Set([...acquire, ...lowTraits])].slice(0, 5);
  const toAcquire = acquireSet.map(label => ({ label }));

  return { existing, toAcquire };
}
...
function CareerCard({ career, tier }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ lineHeight: 1.3 }}>{typeof career === "object" ? career.name : career}</span>
      {typeof career === "object" && career.emerging && (
        <span style={{
          marginLeft: "auto", fontSize: "0.6rem", fontWeight: 700,
          background: "rgba(255,255,255,0.22)", borderRadius: 999,
...
function SkillCard({ title, icon, items, accent, bg }) {
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "1.1rem" }}>
        ...
        <div>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: "0.95rem", fontWeight: 700, color: "#0f2535" }}>{title}</div>
        </div>
      </div>
...
function SocietalOfferingCard({ blurb }) {
  return (
    <div style={{
      background: "linear-gradient(135deg,#0f2535 0%,#0d4a3f 60%,#0c3a5c 100%)",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1rem" }}>
        ...
        <div style={{ fontFamily: "'Fraunces',serif", fontSize: "1rem", fontWeight: 700, color: "#ccfbf1" }}>
          What You're Likely to Offer
        </div>
      </div>
      <p style={{
        fontSize: "0.92rem", lineHeight: 1.85, color: "rgba(255,255,255,0.82)",
        fontStyle: "italic", maxWidth: "60ch", position: "relative",
      }}>
        {blurb}
      </p>
    </div>
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1rem" }}>
        ...
        <div>
          ...
          <div style={{ fontSize: "0.75rem", color: "#b45309" }}>Detected from your response patterns</div>
        </div>
      </div>
      {analysis.insights.map((insight, i) => (
        <div key={i} style={{
...
const ResultsPage = () => {
  ...
  return (
    <>
      ...
      <div style={{
        fontFamily: "'Instrument Sans',sans-serif", color: "#0f2535",
        maxWidth: 1100, margin: "0 auto", padding: "2.5rem 1.5rem 5rem",
        ...
        {/* ── 3. Career Recommendations (2-column) ── */}
        <div className="rl-fade" style={{ animationDelay: "0.18s", marginBottom: "2rem" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: "0.75rem", marginBottom: "1.25rem" }}>
            <span style={{ fontFamily: "'Fraunces',serif", fontSize: "1.35rem", fontWeight: 700 }}>
              Your Career Recommendations
        ...
        {/* ── 4. Societal Offering ── */}
        <div className="rl-fade" style={{ animationDelay: "0.22s" }}>
          <SocietalOfferingCard blurb={societalBlurb} />
        </div>

        {/* ── 5. Skill Cards ── */}
        <div className="rl-fade" style={{ animationDelay: "0.26s", marginBottom: "1.5rem" }}>
          <div style={{ fontFamily: "'Fraunces',serif", fontSize: "1.1rem", fontWeight: 700, marginBottom: "0.3rem" }}>
            Your Skill Landscape
          </div>
          <div style={{ fontSize: "0.78rem", color: "#94a3b8", marginBottom: "1.1rem" }}>
            Based on your trait profile and career direction
          </div>
          <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
            <SkillCard
              title="Existing Skillset"
              icon="✦"
              items={existing.length > 0 ? existing : [{ label: "Strong intrinsic drive" }, { label: "Self-directed learner" }]}
              accent="#0d9488"
              bg="linear-gradient(135deg,#f0fdf4,#ecfeff)"
            />
            <SkillCard
              title="Skillset to Acquire"
              icon="◎"
              items={toAcquire.length > 0 ? toAcquire : [{ label: "Domain-specific expertise" }, { label: "Applied project work" }]}
              accent="#7c3aed"
              bg="linear-gradient(135deg,#faf5ff,#f0f9ff)"
            />
          </div>
        </div>
        ...
        {/* ── 9. Actions ── */}
        <div className="rl-fade" style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginTop: "2rem", animationDelay: "0.5s", alignItems: "center" }}>
          <button
            onClick={() => router.push("/aptitude")}
            style={{
              fontFamily: "'Instrument Sans',sans-serif", fontSize: "0.88rem", fontWeight: 600,
          ...
          <button
            onClick={() => {
              ...
              if (uid) router.push(`/report?uid=${uid}`);
          ...
          <p style={{ fontSize: "0.7rem", color: "#94a3b8", marginLeft: "auto" }}>
            UID: {auth?.currentUser?.uid ?? "NOT LOGGED IN"}
          </p>
        </div>
