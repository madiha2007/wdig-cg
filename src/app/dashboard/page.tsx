...
import { SkillsToAcquireWidget } from "../../components/SkillsToAcquireWidget"; // adjust path as needed
            ...
            {personality.traits.map((trait, i) => (
              <motion.div key={i}
                ...
                style={{
                  ...
                  background: "#fff", border: `1px solid ${personality.color}25`,
      ...
      <div style={{ padding: "0 1.5rem 1.5rem" }}>
        {topCareers.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.44rem" }}>
            {topCareers.slice(0, 5).map((name, i) => (
              <motion.div key={i}
...
function GrowthCard({ needsGainSkills, skillMap }: { needsGainSkills: string[]; skillMap: Record<string, number> }) {
  ...
  const segs: Seg[] = skills.map((k, i) => ({
    key: k, score: skillMap[k] ?? 0,
    color: PALETTE[i % PALETTE.length], lightBg: PALETTE_LIGHT[i % PALETTE_LIGHT.length],
    sliceLen, offset: circumference - i * (sliceLen + gapLen),
  }));
          ...
          <svg width={140} height={140} viewBox="0 0 140 140">
            ...
            {segs.map((seg, i) => (
              <circle key={seg.key} cx={CX} cy={CY} r={R} fill="none"
                stroke={seg.color} strokeWidth={strokeW - 1} strokeLinecap="round"
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        backgroundImage: "linear-gradient(rgba(0, 0, 0, 0.11), rgba(0, 0, 0, 0.11)), url('/bg5.avif')",
...
export default function DashboardPage() {
  ...
  /* ── Build the userProfile object passed to the widget ── */
  const userProfile = {
    name: userData.name,
    thinkingStyle,
    topCareers: topCareers.slice(0, 6),
    existingTraits: existingSkills.slice(0, 5).map(s => s.key),
    secretDream: userData.secret_dream,
  ...
  return (
    <>
      ...
      <motion.div
        ...
        {/* 5 · Skills to Acquire — AI-powered widget with hover insights */}
        <motion.div
          initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }}
        ...
        >
          <SkillsToAcquireWidget
            isAssessed={isAssessed}
            fallbackSkills={skillsToAcquire}
