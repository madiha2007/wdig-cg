...
function getRelevantTypes(careers: string[]): Set<string> {
  ...
  careers.forEach(career => {
    ...
    Object.entries(CAREER_TO_INST_TYPE).forEach(([keyword, mappedTypes]) => {
      if (lower.includes(keyword)) mappedTypes.forEach(t => types.add(t));
    });
  });
  return types;
}
...
export default function Institutes() {
  ...
  async function handleViewInstitute(id: string) {
    ...
    try {
      const res = await fetch(`/api/institutes/${id}/details`);
      setSelectedInstitute(await res.json());
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg relative max-h-[92vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            {detailLoading ? (
              ...
              <div>
                ...
                <div className="p-5">
                  {selectedInstitute.rating && <div className="mb-4"><StarRating rating={selectedInstitute.rating} /></div>}
                  <div className="grid grid-cols-2 gap-3 mb-5">
                    {selectedInstitute.fees && (
                      <div className="bg-sky-50 border border-sky-100 rounded-xl p-3">
                        <p className="text-sky-500 text-[10px] uppercase tracking-wider font-bold">Annual Fees</p>
                  ...
                  <div className="space-y-3 text-sm mb-5">
                    ...
                    {Array.isArray(selectedInstitute.courses) && selectedInstitute.courses.length > 0 && (
                      <div className="flex items-start gap-3 py-2">
                        <span className="text-gray-400 min-w-[90px] text-xs uppercase tracking-wide font-semibold pt-0.5">Courses</span>
                        <div className="flex flex-wrap gap-1">
                          {selectedInstitute.courses.map((c: string) => <span key={c} className="text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded-full font-medium">{c}</span>)}
                        </div>
                      </div>
                    )}
                  </div>
                  {selectedInstitute.website && (
                    <a href={selectedInstitute.website} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl transition-colors text-sm">
                      Visit Official Website
