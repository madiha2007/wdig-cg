...
function HeroBanner() {
  return (
    <div style={{
      ...
      />
      {/* Overlay */}
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, rgba(124,58,237,0.82) 0%, rgba(236,72,153,0.72) 100%)" }} />

      {/* Content */}
      <div style={{ position: "relative", zIndex: 1, textAlign: "center", padding: "80px 24px 90px" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.15)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", borderRadius: 100, padding: "7px 20px", fontSize: 12, fontWeight: 600, backdropFilter: "blur(8px)", marginBottom: 28 }}>
          <Sparkles size={13} /> Powered by O*NET · U.S. Department of Labor
        </div>
        <h1 style={{ fontFamily: "'Lora', Georgia, serif", fontSize: "clamp(36px, 6vw, 72px)", fontWeight: 800, color: "#fff", lineHeight: 1.1, marginBottom: 18, textShadow: "0 2px 20px rgba(0,0,0,0.2)" }}>
          Explore Careers
        </h1>
        <p style={{ fontSize: "clamp(15px, 2vw, 20px)", color: "rgba(255,255,255,0.88)", maxWidth: 560, margin: "0 auto", lineHeight: 1.6, fontWeight: 500 }}>
          Thousands of real career paths with live salary data and growth outlooks
        </p>
      </div>
    </div>
  );
...
export default function ExploreCareersPage() {
  ...
  return (
    <div style={{ background: "#faf9ff", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      {/* ── Search + filters ── */}
      <div style={{ background: "#faf9ff", padding: "40px 24px 32px", position: "relative" }}>
        <div style={{ position: "relative", maxWidth: 860, margin: "0 auto" }}>
          <div style={{ position: "relative", maxWidth: 660, margin: "0 auto 24px", transition: "transform 0.2s", transform: searchFocused ? "scale(1.02)" : "scale(1)" }}>
            <div style={{ display: "flex", alignItems: "center", background: searchFocused ? "rgba(124,58,237,0.08)" : "rgba(124,58,237,0.04)", border: searchFocused ? "1.5px solid #7c3aed" : "1.5px solid rgba(124,58,237,0.2)", borderRadius: 18, padding: "14px 20px", gap: 12, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", transition: "all 0.3s" }}>
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} onFocus={() => setSearchFocused(true)} onBlur={() => setSearchFocused(false)} placeholder="Search career, skill, or keyword..." style={{ flex: 1, border: "none", outline: "none", fontSize: 16, color: "#111827", background: "transparent", fontFamily: "'DM Sans',sans-serif" } as any} />
